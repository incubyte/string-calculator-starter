...
class StringCalculator {
	...
	public String add(String numbers) {
		...
		if (containsNegative(numbers).length() > 0)
			return String.format("Negative numbers not allowed: %s", containsNegative(numbers));
	...
	private void calculateSumOf(String[] numbers) {
		...
		for (String number : numbers) {
			...
			res = Float.sum(res, Float.parseFloat(m));
		}
	}
	...
	private String printFloat() {
		return Float.toString((float) (Math.round(res * 100.0) / 100.0));
	}
